...
- Add file-rotation benchmark coverage (#504). `BenchmarkWriter_Write_WithRotation` in `file/internal/rotate` sets `MaxSize: 4 KiB` + `MaxBackups: 2` + `Compress: false` so rotation fires every ~25 writes and the per-rotation cost is isolable from the write path (delta vs `BenchmarkWriter_Write_SyncOnWriteFalse` captures rename + new file + prune — ≈960 ns/write amortised, ≈24 µs per rotation event). A companion `BenchmarkFileOutput_Write_WithRotation` in the public `file` package uses `MaxSizeMB: 1` (the public API minimum) so rotation is dilute across ~6500 writes per cycle — catches regressions in the `file.Output → rotate.Writer → flush` chain that only surface after a rotate. Both benchmarks include a post-loop `filepath.Glob` assertion that rotation actually fired, so a silent break in the rotation trigger would fail the benchmark instead of reading as a free perf win. Part of #504 AC #1 and master-tracker C-18.
