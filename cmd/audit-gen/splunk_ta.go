...
type splunkTAOptions struct {
	// AppName is the human-facing label shown in Splunk Web's app
	// list. Stamped into app.conf [ui] label and the dashboard XML
	// header. Defaults to "TA-axonops-audit".
	AppName string

	// PackageID is the value Splunk's [package] id and [id] name
	// stanzas advertise; Splunkbase's check_for_valid_package_id
	// requires this to equal the uncompressed directory name
	// (filepath.Base(outDir)). Defaults to the output dir basename.
	PackageID string
...
func generateSplunkTA(outDir string, tax audit.Taxonomy, opts splunkTAOptions) error { //nolint:gocritic // hugeParam: opts is by-value to mirror the option pattern in this command
	...
	if opts.PackageID == "" {
		// Splunkbase's check_for_valid_package_id requires the
		// app.conf [package] id to match the uncompressed
		// directory name. Derive from the output dir so the
		// generated TA always installs cleanly.
		opts.PackageID = filepath.Base(outDir)
	}
...
[install]
state = enabled
# is_configured must be false for Splunkbase publishing — true claims
# the operator has already finished setup, which doesn't apply to a
# generated TA installed for the first time.
is_configured = false
build = 1
...
# [package] id MUST match the uncompressed directory name; the
# generator derives PackageID from filepath.Base(outDir).
[package]
id = {{.PackageID}}
...
# apps can use them.
#
# The global [] stanza declares the default ACL for every knowledge
# object in this TA: world-readable, admin-writable. Splunkbase's
# check_meta_default_write_access rule fails the publish if no
