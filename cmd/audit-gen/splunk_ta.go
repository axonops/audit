...
[{{.Sourcetype}}]
# Index-time JSON extraction. Every key in the cim_change envelope
...
# One eventtype per (category, event_type) pair in the taxonomy.

{{range .Eventtypes -}}
[{{.Name}}]
search = sourcetype="{{$.Sourcetype}}" event_type="{{.EventType}}"
