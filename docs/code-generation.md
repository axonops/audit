...
`audit-gen` is distributed as the
`github.com/axonops/audit/cmd/audit-gen` Go module. Install it with
the standard Go toolchain — there is no published container image
(removed in #953; the Go install path is the canonical surface):

```bash
go install github.com/axonops/audit/cmd/audit-gen@latest
```

For CI builds that need a reproducible binary, pin to a tagged
release (e.g. `@v1.0.0`) and verify the corresponding release
tarball checksum via cosign (see [docs/releasing.md](releasing.md)).
