...
| **TLS 1.3 by default; explicit downgrade required** | `TLSPolicy` zero value enables TLS 1.3 only. Operators wanting TLS 1.2 must opt in with `tls.allow_tls12: true` inside the per-output / per-provider `tls:` block. mTLS supported on every TLS-capable output. | `tls_policy_test.go:TestTLSPolicy_Apply_NilReceiver_DefaultsTLS13` and `TestTLSPolicy_Apply_ZeroValue_DefaultsTLS13`; per-output TLS scenarios live in `tests/bdd/features/webhook_output.feature`, `loki_config.feature`, and `syslog_output.feature`. |
