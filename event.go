...
// and description. Implementations are generated by audit-gen for
// compile-time field safety. For dynamic use without code generation,
// see [NewEvent].
//
// The interface is intentionally minimal. Richer metadata (per-field
// descriptors, categories) lives on the generated builder structs
// directly, accessible via the concrete type.
type Event interface {
	// EventType returns the event type name (e.g., "user_create").
	EventType() string
	// Fields returns the event's field key-value pairs.
	Fields() Fields
	// Description returns the taxonomy description for this event type.
	Description() string
}
...
func (e *basicEvent) Description() string { return e.description }
...
// AuditEvent emits an audit event using this handle's event type.
func (e *EventType) AuditEvent(evt Event) error {
