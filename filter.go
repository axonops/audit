...
type filterState struct {
	...
	// hasEventOverrides is true when at least one per-event override
	// has been registered. Read on the hot path to skip the
	// sync.Map.Load when no overrides exist (the common case).
...
func (f *filterState) isEnabled(eventType string, taxonomy *Taxonomy) bool {
	// Per-event override takes precedence. The hasEventOverrides flag
	// skips the sync.Map.Load when no overrides exist (common case).
