...
// iouringStrategy wraps the low-level ring type to present the
// common strategyImpl interface.
type iouringStrategy struct {
	r *ring
}

func (s *iouringStrategy) kind() Strategy                            { return StrategyIouring }
func (s *iouringStrategy) close() error                              { return s.r.close() }
...
func tryIouring(depth uint32) (strategyImpl, error) {
	...
	return &iouringStrategy{r: r}, nil
}
