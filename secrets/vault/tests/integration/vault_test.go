...
// Package integration tests the Vault provider against a real
// HashiCorp Vault server running in Docker with dev-tls mode.
//
// Prerequisites:
//
...
const (
	vaultAddr  = "https://localhost:8210"
...
func extractCA(t *testing.T) string {
	...
	out, err := exec.Command("docker", "exec", "bdd-vault-1",
...
func newProvider(t *testing.T, caPath string) *vault.Provider {
	t.Helper()
...
func TestResolve_Success(t *testing.T) {
	...
	seedSecret(t, caPath, "secret/data/integration/hmac", map[string]string{
		"salt":      "real-vault-salt-value-32-bytes!!",
		"version":   "v1",
	...
	val, err := p.Resolve(context.Background(), secrets.Ref{
		Scheme: "vault", Path: "secret/data/integration/hmac", Key: "salt",
	})
	require.NoError(t, err)
	assert.Equal(t, "real-vault-salt-value-32-bytes!!", val)
}
...
func TestResolve_PathNotFound(t *testing.T) {
	...
	_, err := p.Resolve(context.Background(), secrets.Ref{
		Scheme: "vault", Path: "secret/data/nonexistent", Key: "key",
	})
	require.Error(t, err)
...
func TestResolve_KeyNotFound(t *testing.T) {
	...
	_, err := p.Resolve(context.Background(), secrets.Ref{
		Scheme: "vault", Path: "secret/data/integration/keytest", Key: "missing",
	})
	require.Error(t, err)
	assert.ErrorIs(t, err, secrets.ErrSecretNotFound)
}
...
func TestResolve_ErrorDoesNotLeakPath(t *testing.T) {
	...
	_, err = p.Resolve(context.Background(), secrets.Ref{
		Scheme: "vault", Path: "secret/data/sensitive/topology", Key: "key",
	})
	require.Error(t, err)
